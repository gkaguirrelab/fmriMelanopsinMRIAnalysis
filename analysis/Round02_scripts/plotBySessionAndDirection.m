    ...
    for jj = 1:length(ROIs)
        ...
        for cc = 1:length(conditions)
            ...
            for kk = 1:length(controls)
                ...
                x = 0:1:13;
                g = errorbar(x+offsets(kk),meanP,semP,'.','color', [0.5 0.5 0.5],'MarkerSize',14);% hold on;
                set(get(get(g,'Annotation'),'LegendInformation'),'IconDisplayStyle','children');
                p(kk) = plot(x+offsets(kk), meanP, '-','MarkerSize',10);
                legendInfo{kk} = ([condition control]);
                hold on
            end
            legend (p,legendInfo, 'Interpreter','none');
            title(['Mean_Across_subjects_' condition '_' hemi '_' ROI],'Interpreter','none')
            ...
            savepath = fullfile (dropbox_dir, 'Averages', 'bySession', conditionFolder);
            if ~exist ('savepath', 'dir')
                mkdir (savepath)
            end
            saveas(firFig, fullfile (dropbox_dir, 'Averages', 'bySession', conditionFolder, ['Mean_' condition '_' hemi '_' ROI]), 'pdf');
            close all;
            clear legendInfo
            clear p
            clear g
            ...
            switch (direction)
                case 'Mel stimuli'
                    ...
                    for ll = 1:length(controls2) ,
                        ...
                        semP = std(dataP,0,2)/sqrt(size(dataP,1));
                        x = 0:1:13;
                        g = errorbar(x+offsets(ll),meanP,semP,'.','color', [0.5 0.5 0.5],'MarkerSize',14);% hold on;
                        set(get(get(g,'Annotation'),'LegendInformation'),'IconDisplayStyle','children');
                        p(kk+ll) = plot(x+offsets(ll+1), meanP, '-','MarkerSize',10);
                        legendInfo{kk+ll} = ([condition control]);
                        hold on
                    end
                    legend (p,legendInfo, 'Interpreter','none');
                    title(['Mean_across_subjects' direction '_' hemi '_' ROI],'Interpreter','none')
                    ...
                    savepath = fullfile (dropbox_dir, 'Averages', 'byDirection', 'Mel');
                    if ~exist ('savepath', 'dir')
                        mkdir (savepath)
                    end
                    saveas(firFig, fullfile (dropbox_dir, 'Averages', 'byDirection', 'Mel', ['Mean_' 'Mel' '_' hemi '_' ROI]), 'pdf');
                    close all;
                    clear legendInfo
                    clear p
                    clear g
                ...
                case 'LMS stimuli'
                    ...
                    savepath = fullfile (dropbox_dir, 'Averages', 'byDirection', 'LMS');
                    if ~exist ('savepath', 'dir')
                        mkdir (savepath)
                    end
                    saveas(firFig, fullfile (dropbox_dir,'Averages', 'byDirection', 'LMS', ['Mean_' 'LMS' '_' hemi '_' ROI]), 'pdf');
                    close all;
                    clear legendInfo
                    clear p
                    clear g
                ...
                case 'Attention Tasks'
                    ...
                    savepath = fullfile (dropbox_dir, 'Averages', 'byDirection', 'AttentionTask');
                    if ~exist ('savepath', 'dir')
                        mkdir (savepath)
                    end
                    saveas(firFig, fullfile (dropbox_dir, 'Averages','byDirection', 'AttentionTask', ['Mean_' 'AttentionTask' '_' hemi '_' ROI]), 'pdf');
                    close all;
                    clear legendInfo
                    clear p
                    clear g
