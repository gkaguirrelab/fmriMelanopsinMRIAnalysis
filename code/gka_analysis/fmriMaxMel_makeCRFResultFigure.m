...
for ss=1:nSubjects
    ...
    mean400LMS=cellfun(@(x) x.meanAmplitude, lms400Cells);
    sems400LMS=cellfun(@(x) x.semAmplitude, lms400Cells);
    sems=sems400LMS(ss,:);
    means=mean400LMS(ss,:);
    ...
    mean400Mel=cellfun(@(x) x.meanAmplitude, mel400Cells);
    sems400Mel=cellfun(@(x) x.semAmplitude, mel400Cells);
    sems=sems400Mel(ss,:);
    means=mean400Mel(ss,:);
        fmriMaxMel_PlotCRF( subplotHandle, [5], means, sems, ...
        'xTickLabels',{'400'},...
